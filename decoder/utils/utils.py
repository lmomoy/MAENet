...
def save_mask(mask, index=0, filename="mask.png"):
    """
    ...
    assert len(mask.shape) == 3, "mask should have 3 dimensions (b, h, w)"
    assert 0 <= index < mask.shape[0], "index out of range"
    
    plt.imshow(mask[index], cmap='gray')
    plt.title(f"Mask Visualization for Batch Index {index}")
    plt.axis('off')
...
def compute_knn_distances(points, K):
    b, n, _ = points.shape
    
    # 计算点到点之间的距离 (b, n, n)，使用torch.cdist计算欧氏距离
    distances = torch.cdist(points, points, p=2)  # (b, n, n)
    
    # 获取每个点到K个最近邻点的距离, torch.topk 返回最小的K个距离（除去自身）
    knn_distances, _ = torch.topk(distances, K + 1, largest=False, dim=-1)  # (b, n, K+1)
    knn_distances = knn_distances[:, :, 1:]  # 排除自身的0距离 (b, n, K)
    
    # mean_distances = torch.mean(knn_distances, dim=-1)  # (b, n)
    std_distances = torch.std(knn_distances, dim=-1)    # (b, n)
    
    std_per_cloud = torch.mean(std_distances, dim=-1)   # (b,)
    
    return std_per_cloud

class Loss_Density(nn.Module):
    def __init__(self, K=5):
        super(Loss_Density, self).__init__()
        self.K = K 

    def forward(self, pred):
        # pred (b, 2048, 3)
        std_pred = compute_knn_distances(pred, self.K)  # (b,)
        # std_gt = compute_knn_distances(gt, self.K)      # (b,)
        loss = torch.mean(std_pred)
